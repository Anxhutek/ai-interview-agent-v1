...
export default function Home() {
  ...
  useEffect(() => {
    if (feedback?.graph) {
      setSelectedNode({
        peer: feedback.graph.entity.name,
        isRoot: true,
        fact: feedback.graph.entity.summary,
        intent_meta: {
          edge_kind: "Candidate Profile",
          cognitive_pattern: "Synthesis of turns",
          why_connected: feedback.graph.entity.knot_narrative
        }
      });
    }
  }, [feedback]);
        ...
        <section className="relative z-10 w-full max-w-xl my-auto animate-slide-up">
          <div className="glass-card rounded-2xl p-6 md:p-8">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-semibold tracking-tight text-zinc-100">Setup Interview Session</h2>
            ...
            <form onSubmit={handleStart} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full mt-6 py-3 rounded-lg bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-600 hover:to-violet-700 text-white font-medium text-sm transition-all flex items-center justify-center shadow-lg shadow-indigo-500/20 active:scale-[0.99] disabled:opacity-50"
              >
                {isLoading ? (
                  <>
        ...
        <section className="relative z-10 w-full max-w-6xl flex-grow grid grid-cols-1 lg:grid-cols-4 gap-6 animate-slide-up">
          ...
          <div className="lg:col-span-1 flex flex-col space-y-4">
            ...
            <div className="glass-card rounded-xl p-4 flex flex-col space-y-3">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Session Context</h3>
              <div>
            ...
            {/* Session Stats */}
            <div className="glass-card rounded-xl p-4 flex flex-col space-y-3">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Metrics</h3>
              <div>
                ...
                <div className="h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${Math.min(((turnsCount) / 8) * 100, 100)}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 text-center">
                <div className="p-2 bg-zinc-900/50 border border-zinc-800/40 rounded-lg">
                  <p className="text-xs text-zinc-400">Turns Done</p>
                  <p className="text-lg font-bold text-zinc-200 mt-0.5">{turnsCount}</p>
                </div>
                <div className="p-2 bg-zinc-900/50 border border-zinc-800/40 rounded-lg">
                  <p className="text-xs text-zinc-400">Min Target</p>
                  <p className="text-lg font-bold text-zinc-200 mt-0.5">8</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 glass-card rounded-xl flex flex-col h-[65vh] md:h-[70vh] overflow-hidden">
            ...
            <div className="px-6 py-4 border-b border-zinc-800/60 bg-zinc-900/40 flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-500">
                {turnsCount < 8 ? `${8 - turnsCount} turns remaining to compute results` : "Minimum turns met. You can wrap up or continue."}
              </span>
            </div>
        ...
        <section className="relative z-10 w-full max-w-6xl flex-grow flex flex-col space-y-6 animate-slide-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="glass-card rounded-2xl p-6 flex flex-col items-center justify-center text-center">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-4">Overall Score</h3>
            ...
            <div className="glass-card rounded-2xl p-6 md:col-span-2 flex flex-col justify-between">
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">Cognitive Profile Synthesis</h3>
                <h4 className="text-lg font-bold text-zinc-100 font-outfit mb-3">{feedback.distilledProfile}</h4>
                <p className="text-sm text-zinc-400 leading-relaxed">{feedback.feedback}</p>
              </div>
          ...
          {feedback.graph && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 glass-card rounded-2xl p-6 flex flex-col">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-4">Breeth Memory Graph</h3>
                ...
                <div className="relative flex-grow min-h-[300px] bg-zinc-950/40 border border-zinc-900 rounded-xl flex items-center justify-center p-4 overflow-hidden">
                  ...
                  <div className="relative w-full max-w-md h-full flex flex-col items-center justify-center py-6">
                    ...
                    >
                      ...
                      <span className="text-xs font-semibold tracking-wide text-zinc-200">{feedback.graph.entity.name}</span>
                      <span className="text-[10px] text-zinc-500 mt-0.5 font-mono">Candidate ID</span>
                    </button>

                    {/* Connection lines using simple relative styles */}
                    <div className="relative w-full grid grid-cols-3 gap-4 mt-16 z-10">
                      {feedback.graph.neighbors.map((neighbor, index) => {
                        ...
                        return (
                          ...
                          >
                            <div className="h-6 w-6 rounded-md bg-zinc-800 border border-zinc-700/50 flex items-center justify-center text-zinc-400 mb-1.5">
                              <span className="text-xs font-bold">{index + 1}</span>
                            </div>
                            <span className="text-xs font-medium text-zinc-200 line-clamp-1">{neighbor.peer}</span>
                            <span className="text-[9px] text-zinc-500 uppercase mt-1 tracking-wider">{neighbor.intent_meta.edge_kind}</span>
                          </button>
                        );
                      })}
                    </div>
                    ...
                    <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-20" xmlns="http://www.w3.org/2000/svg">
                      <line x1="50%" y1="35%" x2="16%" y2="70%" stroke="#6366f1" strokeWidth="2" strokeDasharray="4 4" />
                      <line x1="50%" y1="35%" x2="50%" y2="70%" stroke="#6366f1" strokeWidth="2" strokeDasharray="4 4" />
                      <line x1="50%" y1="35%" x2="84%" y2="70%" stroke="#6366f1" strokeWidth="2" strokeDasharray="4 4" />
                    </svg>
                  </div>
                </div>
              </div>
                ...
                <div className="glass-card rounded-2xl p-6 flex-grow flex flex-col justify-between">
                  <div>
                    <div className="flex items-center space-x-2 pb-3 border-b border-zinc-800/60 mb-4">
                      ...
                      <h3 className="font-semibold text-sm text-zinc-300">Memory Node Inspector</h3>
                    </div>
                    ...
                    {selectedNode ? (
                      <div className="space-y-4 animate-fade-in">
                        ...
                        <div>
                          <p className="text-xs text-zinc-500 uppercase tracking-wide">Extracted Fact</p>
                          <p className="text-sm text-zinc-300 mt-1 leading-relaxed bg-zinc-900/30 border border-zinc-850 p-2.5 rounded-lg">
                            {selectedNode.fact}
                          </p>
                        </div>

                        <div>
                          <p className="text-xs text-zinc-500 uppercase tracking-wide">Cognitive Pattern Pattern</p>
                          <span className="inline-block mt-1.5 px-2.5 py-1 text-xs font-medium text-violet-400 bg-violet-950/30 border border-violet-900/40 rounded-md">
                            {selectedNode.intent_meta.cognitive_pattern}
                          </span>
                        </div>

                        <div>
                          <p className="text-xs text-zinc-500 uppercase tracking-wide">Breeth Logical Inference</p>
