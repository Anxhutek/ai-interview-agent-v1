...
|--------|----------|--------------|-----------------|--------|
| `POST` | `/api/interview/start` | `{ "candidateId": "string", "candidateName": "string" }` | `{ "sessionId": "string", "firstQuestion": "string" }` | Planned |
| `POST` | `/api/interview/message` | `{ "sessionId": "string", "message": "string" }` | `{ "reply": "string", "isFinished": "bool" }` | Planned |
| `GET`  | `/api/interview/feedback` | `?sessionId=abc` | `{ "feedback": "string", "score": "int", "distilledProfile": "string" }` | Planned |

---

## 🧠 Breeth API Memory Layer Integration

The application integrates with the Breeth memory layer to store candidate responses and extract traits, decisions, and distilled profiles.
